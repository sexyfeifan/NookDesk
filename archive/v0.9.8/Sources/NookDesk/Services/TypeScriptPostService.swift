...
final class TypeScriptPostService {
        ...
        if fm.fileExists(atPath: url.path) {
            if let existingContent = try? String(contentsOf: url, encoding: .utf8) {
                ...
                if hasContent && parsed.isEmpty {
                    // 文件有内容（>1KB 且有 id:）但解析出 0 篇文章 → 拒绝覆盖
    ...
    func restoreFromGit(project: BlogProject) throws -> [TSPPost] {
        ...
        var posts = (try? loadPosts(from: project)) ?? []
        // 如果 git HEAD 里没有文章（被 NookDesk 覆盖过），从原始仓库恢复
        if posts.isEmpty {
            let url = postsFilePath(in: project)
            if let content = try? String(contentsOf: url, encoding: .utf8), content.count < 1000 {
                // 文件太小，可能被清空了，从原始仓库下载
                if let remoteURL = URL(string: "https://raw.githubusercontent.com/guokaigdg/animal-island-blog/main/src/pages/Home/posts.ts"),
                   ...
                   let remoteContent = String(data: data, encoding: .utf8) {
                    ...
                    posts = (try? loadPosts(from: project)) ?? []
                }
            }
        }
        return posts
            ...
            if inString {
                ...
                if stringDelim == "`" && c == "$" {
                    ...
                    if next < text.endIndex && text[next] == "{" {
                        ...
                        var templateDepth = 1
                        while i < text.endIndex && templateDepth > 0 {
                            if text[i] == "{" { templateDepth += 1 }
                            if text[i] == "}" { templateDepth -= 1 }
                            if text[i] == "\\" {
                                i = text.index(after: i)
                                if i < text.endIndex { i = text.index(after: i) }
