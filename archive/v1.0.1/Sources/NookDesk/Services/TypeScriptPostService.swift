...
final class TypeScriptPostService: @unchecked Sendable {
    ...
    func restoreFromGit(project: BlogProject) throws -> [TSPPost] {
        ...
        // 1. Download remote posts to temp file, parse, then clean up
        ...
        if let remoteURL = URL(string: "https://raw.githubusercontent.com/guokaigdg/animal-island-blog/main/src/pages/Home/posts.ts"),
           ...
           let content = String(data: data, encoding: .utf8) {
            let tmpDir = URL(fileURLWithPath: NSTemporaryDirectory())
                .appendingPathComponent("nookdesk-restore-\(UUID().uuidString.prefix(8))")
            try fm.createDirectory(at: tmpDir, withIntermediateDirectories: true)
            let tmpPostsFile = tmpDir.appendingPathComponent("posts.ts")
            try content.write(to: tmpPostsFile, atomically: true, encoding: .utf8)

            if let json = try? readPostsViaNode(postsFile: tmpPostsFile, cwd: project.rootURL) {
                remotePosts = (try? parseJSONPosts(json)) ?? []
            }
            try? fm.removeItem(at: tmpDir)
        }

        // 2. Load existing local posts
        let localPosts = (try? loadPosts(from: project)) ?? []

        // 3. Merge: local takes priority (overwrites remote with same id)
        ...
        // 4. Save merged result directly (bypass savePosts safety check)
        let ts = renderPostsFile(result)
        let dir = url.deletingLastPathComponent()
        try fm.createDirectory(at: dir, withIntermediateDirectories: true)
        try ts.write(to: url, atomically: true, encoding: .utf8)
