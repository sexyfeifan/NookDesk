                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    HStack(spacing: 8) {
                        NookButton(.primary, size: .small, label: "克隆到所选目录") {
                            ...
                            cloneError = nil
                            Task {
                                await viewModel.cloneProjectFromGitHub(url: cloneURL)
                                isCloning = false
                                if viewModel.statusText.contains("失败") || viewModel.statusText.contains("错误") {
                                    cloneError = viewModel.statusText
                                }
                            }
                        }
