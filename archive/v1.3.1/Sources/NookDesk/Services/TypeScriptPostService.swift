...
final class TypeScriptPostService: @unchecked Sendable {
    ...
    func savePosts(_ posts: [TSPPost], to project: BlogProject, force: Bool = false) throws {
        ...
        let ts = try renderPostsFile(posts)
    ...
    func restoreFromGit(project: BlogProject) async throws -> [TSPPost] {
        let remoteURLs = [
        ...
        // 注意：如果本地读取失败，localPosts 为空，合并后只有远程文章
        // 这是可接受的，因为恢复的目的是恢复内容
        ...
        try savePosts(result, to: project, force: true)
    ...
    private func findMatchingBrace(in content: String, from openBrace: String.Index) -> String.Index? {
        ...
        while i < content.endIndex {
            ...
            if inString {
                ...
                if c == stringDelim && templateDepth == 0 { inString = false }
                if stringDelim == "`" && c == "$" {
                ...
                if stringDelim == "`" && templateDepth > 0 {
        ...
        for pattern in patterns {
            if let regex = try? NSRegularExpression(pattern: pattern, options: []),
               ...
               let range = Range(match.range(at: 1), in: block) {
                return unescapeString(String(block[range]))
            }
        }
        return nil
    }
    ...
    private func parseSections(from block: String) -> [TSPSection] {
        var sections: [TSPSection] = []
        // 锚定到 sections: [ 而不是 body 中可能包含的 "sections:"
        ...
        // 确保紧跟的是 [ 而不是其他字符
        guard let arrayOpen = afterSectionsStr.firstIndex(where: { $0 == "[" || $0 == "\n" }),
              afterSectionsStr[arrayOpen] == "[",
              let arrayClose = findMatchingBracket(in: afterSectionsStr, from: arrayOpen) else { return [] }
        let arrayContent = String(afterSectionsStr[afterSectionsStr.index(after: arrayOpen)..<arrayClose])

        var searchIdx = arrayContent.startIndex
        while searchIdx < arrayContent.endIndex {
            guard let objOpen = arrayContent[searchIdx...].firstIndex(of: "{") else { break }
            guard let objClose = findMatchingBrace(in: arrayContent, from: objOpen) else { break }
            let objContent = String(arrayContent[objOpen...objClose])

            if let heading = extractField("heading", from: objContent) {
                let paragraphs = parseStringArray("paragraphs", from: objContent)
                sections.append(TSPSection(heading: heading, paragraphs: paragraphs))
            }

            searchIdx = arrayContent.index(after: objClose)
        }
        ...
        if let regex = try? NSRegularExpression(pattern: pattern, options: []) {
            ...
            for match in matches {
                if let range = Range(match.range(at: 1), in: arrayContent) {
                    items.append(unescapeString(String(arrayContent[range])))
                }
            }
        }
        return items
    ...
    func renderPostsFile(_ posts: [TSPPost]) throws -> String {
        var lines: [String] = []
        ...
        try validateRenderedOutput(result)
        return result
    }

    private func validateRenderedOutput(_ content: String) throws {
        for (lineNum, line) in content.components(separatedBy: "\n").enumerated() {
            let trimmed = line.trimmingCharacters(in: .whitespaces)
            if trimmed == "," {
                throw NSError(domain: "NookDesk", code: 99, userInfo: [
                    NSLocalizedDescriptionKey: "渲染输出错误：第 \(lineNum + 1) 行有孤立逗号，可能是渲染 bug。"
                ])
            }
        }
    }
    ...
    private func escapeDQ(_ s: String) -> String {
        s.replacingOccurrences(of: "\\", with: "\\\\")   // \ → \\
         ...
         .replacingOccurrences(of: "\r", with: "\\r")    // CR → \r
         .replacingOccurrences(of: "\t", with: "\\t")    // tab → \t
         .replacingOccurrences(of: "`", with: "\\`")     // ` → \`
         .replacingOccurrences(of: "$", with: "\\$")     // $ → \$
    }

    /// 单字符遍历反转义（避免多步替换的顺序问题）
    private func unescapeString(_ s: String) -> String {
        var result = ""
        var i = s.startIndex
        while i < s.endIndex {
            let c = s[i]
            if c == "\\", i < s.index(before: s.endIndex) {
                ...
                case "`": result.append("`")
                case "$": result.append("$")
                default: result.append(c); result.append(s[next])
                }
