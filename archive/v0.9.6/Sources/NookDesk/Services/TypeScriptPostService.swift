...
final class TypeScriptPostService {
    ...
    func savePosts(_ posts: [TSPPost], to project: BlogProject) throws {
        ...
        // 安全检查：如果合并后文章数为 0 且文件已有内容，拒绝覆盖
        if merged.isEmpty {
            let fm = FileManager.default
            if fm.fileExists(atPath: url.path) {
                let existingContent = try? String(contentsOf: url, encoding: .utf8)
                if let content = existingContent, content.contains("id:") {
                    // 文件有文章但解析失败，拒绝覆盖
                    throw NSError(domain: "NookDesk", code: 1, userInfo: [
                        NSLocalizedDescriptionKey: "安全保护：检测到 posts.ts 有内容但解析失败，拒绝覆盖。请使用「恢复」按钮恢复原始内容。"
                    ])
                }
            }
        }
