...
struct SettingsView: View {
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            settingsRow("后端类型") {
                HStack(spacing: 8) {
                    ...
                    NookButton(.default, size: .small, label: "检查版本") {
                        viewModel.runHugoVersionCheck()
                    }
                }
            }

            settingsRow("Hugo 可执行命令") {
                NookInput("hugo", text: $viewModel.project.hugoExecutable)
            }

            settingsRow("文章目录") {
                NookInput("content/post", text: $viewModel.project.contentSubpath)
            }
    ...
    private var themeSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            settingsRow("默认关键词") {
                NookInput("hugo, blog", text: $viewModel.config.params.keywords)
            }
