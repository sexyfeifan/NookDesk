...
struct WritingView: View {
    ...
    .onChange(of: editingTSPost?.title) { newTitle in
        if let id = selectedTSPostID, let idx = tsPosts.firstIndex(where: { $0.id == id }) {
            tsPosts[idx].title = newTitle ?? ""
        }
    }
    .onChange(of: editingTSPost?.tag) { newTag in
        if let id = selectedTSPostID, let idx = tsPosts.firstIndex(where: { $0.id == id }) {
            tsPosts[idx].tag = newTag ?? ""
        }
    }
    .onChange(of: editingTSPost?.date) { newDate in
        if let id = selectedTSPostID, let idx = tsPosts.firstIndex(where: { $0.id == id }) {
            tsPosts[idx].date = newDate ?? ""
        }
    }
    }
    ...
    private func tsEditorStatusBar(post: TSPPost) -> some View {
        HStack(spacing: 8) {
    ...
    private func tsEditorArea(post: TSPPost) -> some View {
        NookCard {
    ...
    private func tsInspector(post: TSPPost) -> some View {
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                NookButton(.default, size: .small, label: "+ 章节") {
                    ...
                    post.sections.append(TSPSection(heading: "新章节", paragraphs: [""]))
                    editingTSPost = post
                }
            }
