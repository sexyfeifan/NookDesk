    ...
    private func stagePublishFiles(project: BlogProject) throws -> ProcessResult {
        ...
        do {
            ...
            if output.contains("ignored by one of your .gitignore files") {
                ...
                let _ = try? runner.run(command: "git", arguments: ["add", "--intent-to-add", "."], in: project.rootURL)
                let _ = try? runner.run(command: "git", arguments: ["reset", "HEAD", ".nookdesk.local.json", "NookDesk/", "NookDeskStorage/"], in: project.rootURL)
