...
struct LogEntry: Identifiable {
    ...
    var formatted: String {
        ...
        let icon = success ? "✅" : "❌"
        return "[\(time)] \(icon) [\(category)] \(action): \(detail)"
    }
}
...
final class AppViewModel: ObservableObject {
    ...
    @Published var globalLogs: [LogEntry] = []
    ...
    func log(category: String, action: String, detail: String, success: Bool = true) {
        let entry = LogEntry(timestamp: Date(), category: category, action: action, detail: detail, success: success)
        globalLogs.insert(entry, at: 0)
        if globalLogs.count > 200 { globalLogs = Array(globalLogs.prefix(200)) }
        print("[NookDesk] \(entry.formatted)")
    }
    ...
    func setProjectRootPath(_ path: String) {
        ...
        log(category: "系统", action: "设置项目路径", detail: trimmed)
        loadAll()
    }
    ...
    private func runUnifiedPublishWorkflow(operation: String) {
        ...
        log(category: "发布", action: "开始发布", detail: "远程: \(remote)")
    ...
    private func executeUnifiedPublishPipeline() async throws -> String {
        ...
        do {
            ...
            if msg.contains("nothing to commit") || msg.contains("no changes added") {
                logs.append("== 提交并推送 ==\n没有需要发布的变更。如果你刚修改了文章，请先在写作页面点击「保存」。")
            } else {
                throw error
            }
        }
    ...
    func cloneProjectFromGitHub(url: String) {
        ...
        Task {
            defer { isBusy = false }
            do {
                log(category: "系统", action: "克隆项目", detail: trimmedURL)
    ...
    func preflightChecks() -> [PublishCheck] {
        ...
        checks.append(
            PublishCheck(
                ...
                detail: workflowExists
                    ...
                    : "未检测到 .github/workflows/\(workflowFileName)。点击「生成 Workflow」自动创建。",
        ...
        if classic.isEmpty, fine.isEmpty {
            tokenDetail = "未配置。可选但推荐配置以启用自动推送和 Actions 状态查询。"
