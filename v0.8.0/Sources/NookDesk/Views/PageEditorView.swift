...
struct PageEditorView: View {
    ...
    private var homeURL: URL {
        if isAstroBackend {
            return viewModel.project.rootURL.appendingPathComponent("src/pages/index.astro")
        }
        return viewModel.project.rootURL.appendingPathComponent("src/pages/Home/Home.tsx")
    }
    ...
    // MARK: - Astro Page Editor

    private var astroPageEditor: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                astroHeaderCard
                astroSiteConfigSection
                astroProjectsSection
                astroFriendsSection
        ...
        .onAppear { loadAstroSections() }
    }

    private var astroHeaderCard: some View {
        NookCard(color: .appBlue) {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text("在这里修改博客的站点配置、项目展示和友链。每个区域修改后点击「保存」即可写入文件。")
    ...
    private var astroSiteConfigSection: some View {
        NookCard(color: .appYellow) {
            VStack(alignment: .leading, spacing: 12) {
                ...
                saveButton { saveAstroSiteConfig() }
            }
        }
    }

    private var astroProjectsSection: some View {
        NookCard(color: .appGreen) {
            VStack(alignment: .leading, spacing: 12) {
                ...
                saveButton { saveAstroProjects() }
            }
        }
    }

    private var astroFriendsSection: some View {
    ...
    // MARK: - Astro Load/Save

    private func loadAstroSections() {
        ...
        if let content = try? String(contentsOf: root.appendingPathComponent("src/layouts/Base.astro"), encoding: .utf8) {
            if let titleMatch = extractAstroText(in: content, pattern: #"<title>([^<]+)</title>"#) {
                astroSiteTitle = titleMatch
            }
            if let descMatch = extractAstroText(in: content, pattern: #"content="([^"]+)"[^>]*name="description""#) {
        ...
        if let content = try? String(contentsOf: root.appendingPathComponent("src/pages/projects.astro"), encoding: .utf8) {
            astroProjects = parseAstroProjects(from: content)
        }

        if let content = try? String(contentsOf: root.appendingPathComponent("src/pages/friends.astro"), encoding: .utf8) {
            astroFriends = parseAstroFriends(from: content)
        }

        statusMessage = "已加载 Astro 页面数据。"
    }

    private func saveAstroSiteConfig() {
        let root = viewModel.project.rootURL
        ...
        content = replaceAstroText(in: content, pattern: #"<title>[^<]+</title>"#, replacement: "<title>\(astroSiteTitle)</title>")
    ...
    private func saveAstroProjects() {
        let root = viewModel.project.rootURL
        ...
        if let oldBlock = extractAstroArrayBlock(in: content, marker: "const projects") {
            content = content.replacingOccurrences(of: oldBlock, with: newArray)
        }

        writeBackAstro(content, fileURL: fileURL, section: "项目展示")
    }

    private func saveAstroFriends() {
        let root = viewModel.project.rootURL
        ...
        if let oldBlock = extractAstroArrayBlock(in: content, marker: "const friends") {
            content = content.replacingOccurrences(of: oldBlock, with: newArray)
        }

        writeBackAstro(content, fileURL: fileURL, section: "友情链接")
    }

    // MARK: - Astro Helpers

    private func extractAstroText(in text: String, pattern: String) -> String? {
    ...
    private func replaceAstroText(in text: String, pattern: String, replacement: String) -> String {
    ...
    private func extractAstroArrayBlock(in content: String, marker: String) -> String? {
    ...
    private func parseAstroProjects(from content: String) -> [AstroProjectItem] {
        guard let block = extractAstroArrayBlock(in: content, marker: "const projects") else { return [] }
    ...
    private func parseAstroFriends(from content: String) -> [AstroFriendItem] {
        guard let block = extractAstroArrayBlock(in: content, marker: "const friends") else { return [] }
    ...
    private func writeBackAstro(_ content: String, fileURL: URL, section: String) {
        do {
    ...
    // MARK: - Vite Page Editor (original)

    private var vitePageEditor: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                headerCard
                brandSection
